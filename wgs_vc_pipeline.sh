...
sample=$(basename "${align}"/*.sam | cut -d '_' -f1)
samtools view -@ 1 -b "${align}"/*.sam | samtools sort -@ 1 -o "${align}/${sample}_sorted.bam"

# Mark duplicates using sambamba
sample=$(basename "${align}"/*_sorted.bam | cut -d '_' -f1)

# Full path to the first sorted BAM file
bam_file="${align}/${sample}_sorted.bam"
sambamba markdup -r -t 5 "$bam_file" "${align}/$sample"_rmdup.bam

# Call variants using freebayes
freebayes -f ${index}/*.fna -p 1 ${align}/*_rmdup.bam > ${align}/variants.vcf
